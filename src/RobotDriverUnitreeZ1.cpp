...
public:
    ...
    //std::shared_ptr<_z1_controller_wrapper> z1_controller_;
    Impl()
    {
...
RobotDriverUnitreeZ1::RobotDriverUnitreeZ1(std::atomic_bool *st_break_loops, const MODE &mode,
    ...
    gripper_attached_{gripper_attached}, finish_echo_robot_state_{false}
{
    ...
    impl_       = std::make_shared<RobotDriverUnitreeZ1::Impl>();
    ...
    */
    ...
    current_status_ = RobotDriver::STATUS::IDLE;
...
std::tuple<VectorXd, VectorXd> RobotDriverUnitreeZ1::_compute_control_inputs(VectorXd &q,
...
void RobotDriverUnitreeZ1::_show_status()
{
    if (verbosity_)
        std::cerr<<status_msg_<<std::endl;
}

void RobotDriverUnitreeZ1::_update_q_for_numerical_integration()
{
...
void RobotDriverUnitreeZ1::_set_driver_mode(const MODE &mode)
{
    switch (mode){

    case RobotDriver::MODE::None:
        mode_ = mode;
        break;
    case RobotDriver::MODE::PositionControl:
        mode_ = mode;
        break;
    case RobotDriver::MODE::VelocityControl:
        throw std::runtime_error("RobotDriverUnitreeZ1::_set_driver_mode: VelocityControl is unsupported");
        break;
    case RobotDriver::MODE::ForceControl:
        throw std::runtime_error("RobotDriverUnitreeZ1::_set_driver_mode: ForceControl is unsupported");
        break;
    }
}

void RobotDriverUnitreeZ1::_update_robot_state()
{
...
void RobotDriverUnitreeZ1::_start_echo_robot_state_mode()
{
...
/**
 * @brief RobotDriverUnitreeZ1::_start_echo_robot_state_mode_thread
 */
void RobotDriverUnitreeZ1::_start_echo_robot_state_mode_thread()
{
    ...
    echo_robot_state_mode_thread_ = std::thread(&RobotDriverUnitreeZ1::_start_echo_robot_state_mode, this);
}

void RobotDriverUnitreeZ1::_finish_echo_robot_state()
{
...
void RobotDriverUnitreeZ1::_start_joint_position_control_mode()
{
...
void RobotDriverUnitreeZ1::_start_joint_position_control_thread()
{
    ...
    joint_position_control_mode_thread_ = std::thread(&RobotDriverUnitreeZ1::_start_joint_position_control_mode, this);
...
/**
 * @brief RobotDriverUnitreeZ1::initialize This method starts the corresponding threads for kinematic control.
...
void RobotDriverUnitreeZ1::set_target_gripper_position(const double &target_gripper_position)
{
    target_gripper_position_ = target_gripper_position;
}

void RobotDriverUnitreeZ1::_finish_motion()
{
...
void RobotDriverUnitreeZ1::_move_robot_to_target_joint_positions(const VectorXd &q_target, const double &gain, std::atomic_bool *break_loop)
{
    _update_q_for_numerical_integration();
    VectorXd q_dot;
...
VectorXd RobotDriverUnitreeZ1::get_joint_forces()
{
    return tau_measured_;
}

double RobotDriverUnitreeZ1::get_gripper_position()
{
    return gripper_position_measured_;
}

void RobotDriverUnitreeZ1::move_robot_to_forward_position_when_initialized(const bool &flag)
{
    move_robot_to_forward_position_when_initialized_ = flag;
}

void RobotDriverUnitreeZ1::move_robot_to_target_joint_positions(const VectorXd &q_target)
{
...
void RobotDriverUnitreeZ1::set_target_joint_positions(const VectorXd &target_joint_positions_rad)
{
